...
p3<-
  with(data=power.1,plot(y=as.numeric(Sub_metering_1),x=as.Date(datetime),ylab="Energy sub metering",xlab="",col="grey9",type="l",cex=.7))+
  with(data=power.1,lines(y=as.numeric(Sub_metering_2),x=as.Date(datetime),col="red",type="l",cex=.7))+
  with(data=power.1,lines(y=as.numeric(Sub_metering_3),x=as.Date(datetime),col="blue",type="l",cex=.7))+
